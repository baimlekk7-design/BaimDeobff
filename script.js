// ==================== BaimDeobf v2 - Lua Deobfuscator + XOR Decoder ====================
...
function tryDecodeSimpleXor(code) {
    // Cari string literal panjang dengan escape \ddd (kutip tunggal/ganda)
    ...
    // Konversi escape \ddd menjadi array byte
    const byteArray = [];
    let i = 0;
    ...
    for (let idx = 0; idx < byteArray.length; idx++) {
        ...
        key = byte; // update dengan byte asli
    }

    // Kembalikan hasil decode (kode Lua asli)
    return decodedStr;
}
...
function deobfuscate(code, options) {
    ...
    // Auto decode XOR loop jika diaktifkan
    if (options.autoXor) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Contoh kode XOR (diambil dari potongan yang kamu kirim)
