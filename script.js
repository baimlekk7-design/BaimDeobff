...
// ---------- Utility: extract strings & comments ----------
...
function decodeWeAreDevsString(encoded, charToVal) {
    ...
    for (let i = 0; i < encoded.length; i++) {
        const ch = encoded[i];
        if (ch === '=') {
            if (D >= 2) bytes.push(Math.floor(X / 65536));
            if (D >= 1) bytes.push(Math.floor((X % 65536) / 256));
            break;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Contoh kode XOR (disingkat agar tidak terlalu panjang)
